...
def after_build(source, target, env):
	shutil.copy(firmware_source, 'bin/withotanodisp/firmware_%s_%s.bin' % (kind, version))

def after_parts_build(source, target, env):
	shutil.copy(partitions_source, 'bin/withotanodisp/partitions_%s_%s.bin' % (kind, version))

#def after_fs_build(source, target, env):
#	shutil.copy(littlefs_source, 'bin/withotanodisp/littlefs_%s_%s.bin' % (kind, version))
