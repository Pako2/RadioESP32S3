...
def after_build(source, target, env):
	shutil.copy(firmware_source, 'bin/withota/firmware_%s_%s.bin' % (kind, version))

#def after_parts_build(source, target, env):
#	shutil.copy(partitions_source, 'bin/withota/partitions_%s_%s.bin' % (kind, version))

#def after_fs_build(source, target, env):
#	shutil.copy(littlefs_source, 'bin/withota/littlefs_%s_%s.bin' % (kind, version))
