...
def after_build(source, target, env):
	shutil.copy(firmware_source, 'bin/noota/firmware_%s_%s.bin' % (kind, version))

#def after_parts_build(source, target, env):
#	shutil.copy(partitions_source, 'bin/noota/partitions_%s_%s.bin' % (kind, version))

def after_fs_build(source, target, env):
	shutil.copy(littlefs_source, 'bin/noota/littlefs_%s_%s.bin' % (kind, version))
